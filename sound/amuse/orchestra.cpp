...
sound_entry mouthao(instruction ii)
{
    ...
    ahe->p(.5, 1);
    bu_ptr ah = U<harmonics>(P<still>(ii.f), ahe, 7, ii.p.get(0));
    bu_ptr bh = U<harmonics>(P<still>(ii.f), P<punctual>(.5, 0), 7, ii.p.get(0));
    bu_ptr s = U<cross>(std::move(ah), std::move(bh),
            P<movement>(P<punctual>(0, 1), ii.d));
...
sound_entry fmfi(instruction ii)
{
    ...
    bu_ptr ms = U<wave>(P<still>(ii.f * (ii.p.get(2)+.5)), P<sine>(0));
    bu_ptr s = U<fm>(std::move(ms), ie, P<still>(ii.f));
    return sound_entry(P<attack>(a, ii.h, ii.d, std::move(s)), a);
}
...
sound_entry guitar(instruction ii)
{
    ...
    bu_ptr s = U<karpluss_strong>(P<still>(ii.f),
...
sound_entry shaperw(instruction ii)
{
    ...
    bu_ptr wc = U<wave>(P<still>(ii.f), we);
...
sound_entry fspread(instruction ii)
{
    ...
    bu_ptr s = U<chorus>(P<still>(ii.f, .3), t, P<sine>(0), 4);
    return sound_entry(P<attack>(a, ii.h, ii.d, std::move(s)), a);
}
